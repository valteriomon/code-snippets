...
async function handle (event, ctx, cb) {
    let response;
    if (event.body.length) {
        const rawBlocks = readMyClippingsFile(event.body);
        const parsedBlocks = parseRawBlocks(rawBlocks);
        const books = groupToBooks(parsedBlocks);
        
        let markdownContent = "";
        for(let i = 0; i < books.length; i++) {
            markdownContent += "## " + books[i].title + "\n\n";
            const annotations = books[i].annotations;
            for(let j = 0; j < annotations.length; j++) {
                markdownContent += annotations[j].content + "\n\n";
            }
        }
